...
class Model_Questbook extends Model {
  ...
  public function addMsg($firstname, $message) {
    ...
    } elseif(!preg_match('/^[a-zа-яё]{2,110}$/ui', $firstname)) {
    ...
    $ts = time();
$sql = "INSERT INTO `questbook`(`id`, `owner_name`, `date_created`, `text`) VALUES
                               ('',:owner_name,:date_created,:_text)";
    $add_msg = $this->database->prepare($sql);

    $add_msg->execute(array(':owner_name' => 'sadsadas',
    	                    ':date_created' => '12321',
    	                    ':_text' => 'sadsdas'));
    return array('err'=>false,'msg_id'=>$this->database->lastInsertId(),'firstname'=>$firstname,'date_created'=>$ts,'text'=>$message);
  }
